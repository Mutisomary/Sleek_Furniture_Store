...
def delete_item(item_id):
    if current_user.id == 1:
        try:
            ...
            deleted_item_id = item_to_delete.id
            db.session.delete(item_to_delete)
            db.session.commit()

            # Update the IDs of all subsequent items
            subsequent_items = Product.query.filter(Product.id > deleted_item_id).order_by(Product.id).all()
    ...
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                print(e)
                flash('Order {order_id} not Updated')
                return redirect('/view-orders')
